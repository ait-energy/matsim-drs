...
class NearestRequestsFinderTest {
    ...
    void testEmptyStreamsIntersection(){
        assertEquals(0,NearestRequestsFinder.getIntersection(new CarpoolingConfigGroup("cfgGroup"),originNearRequests.stream(),destinationNearRequests.stream(),temporalNearRequests.stream()).size());
    }
    ...
    void testIntersectionInAllStreams(){
        ...
        destinationNearRequests.add(request2);
        temporalNearRequests.add(request3);
        assertEquals(1,NearestRequestsFinder.getIntersection(new CarpoolingConfigGroup("cfgGroup"),originNearRequests.stream(),destinationNearRequests.stream(),temporalNearRequests.stream()).size());
    }
}