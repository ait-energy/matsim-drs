...
public class BestRequestFinder {
    ...
    public BestRequestFinder(LeastCostPathCalculator leastCostPathCalculator, double detourFactorWeight, double driverWaitingTimeWeight, double passengerWaitingTimeWeight) {
    ...
    public CarpoolingRequest findBestRequest(CarpoolingRequest driverRequest, ArrayList<CarpoolingRequest> filteredPassengersRequests) {
        ...
        LeastCostPathCalculator.Path  originalPath = leastCostPathCalculator.calcLeastCostPath(driverOrigin,driverDestination,0,null,null);
        ...
        for (CarpoolingRequest passengerRequest : filteredPassengersRequests) {
            ...
            LeastCostPathCalculator.Path  pathToCustomer = leastCostPathCalculator.calcLeastCostPath(driverOrigin,passengerOrigin,0,null,null);
            double t1 = pathToCustomer.travelTime;
            LeastCostPathCalculator.Path  pathWithCustomer = leastCostPathCalculator.calcLeastCostPath(passengerOrigin,passengerDestination,0,null,null);
            double t2 = pathWithCustomer.travelTime;
            LeastCostPathCalculator.Path  pathAfterCustomer = leastCostPathCalculator.calcLeastCostPath(passengerDestination,driverDestination,0,null,null);
            ...
            double driverWaitingTime = Math.max(passengerRequest.getSubmissionTime() - (driverRequest.getSubmissionTime()+t1),0);
            double passengerWaitingTime = Math.max((driverRequest.getSubmissionTime()+t1) - passengerRequest.getSubmissionTime(),0);

            double score = (-detourFactorWeight*detourTime)+(-driverWaitingTimeWeight*driverWaitingTime)+(-passengerWaitingTimeWeight*passengerWaitingTime);
