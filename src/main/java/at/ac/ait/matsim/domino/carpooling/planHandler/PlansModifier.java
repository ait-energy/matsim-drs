...
public class PlansModifier implements StartupListener, ReplanningListener {
    ...
    private void preplanDay(ControlerEvent event) {
        ...
        CarpoolingOptimizer optimizer = new CarpoolingOptimizer(network,cfgGroup,population,router,event.getServices().getIterationNumber());
    ...
    private void modifyPlans(CarpoolingRequest driverRequest,CarpoolingRequest riderRequest,PopulationFactory factory,RoutingModule router ) {
                ...
                if (planElement instanceof Activity) {
                    if (!(CarpoolingUtil.getLinkageActivityToRiderRequest((Activity) planElement)==null)) {
                        if (CarpoolingUtil.getLinkageActivityToRiderRequest((Activity) planElement)==riderRequest.getId().toString()){
                            ...
                            LOGGER.warn("Before matching "+ riderRequest.getPerson().getId().toString()+"'s departure is at "+ ((Activity) planElement).getEndTime().seconds());
                            ((Activity) planElement).setEndTime(pickupTime);
                            CarpoolingUtil.removeLinkageActivityToRiderRequest((Activity) planElement);
                            LOGGER.warn("After matching "+ riderRequest.getPerson().getId().toString()+"'s departure is at "+ ((Activity) planElement).getEndTime().seconds());
                            break;
                        }
                    }
                }
