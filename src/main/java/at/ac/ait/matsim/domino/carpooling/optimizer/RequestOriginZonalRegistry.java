...
public class RequestOriginZonalRegistry {
   ...
   // private final Map<Id<Zone>, List<Zone>> zonesSortedByDistance;
    ...
    public RequestOriginZonalRegistry(Network network, ZonalSystem zonalSystem, Map<Id<Zone>, ArrayList<CarpoolingRequest>>  requestsInZones) {
...
/*    public Stream<CarpoolingRequest> findNearestRequests(Node node, int minCount) {
    ...
    private Id<Zone> getZoneId(CarpoolingRequest request) {
        return zonalSystem.getZone(NetworkUtils.getNearestNode(network,request.getOrigin())).getId();
    }
