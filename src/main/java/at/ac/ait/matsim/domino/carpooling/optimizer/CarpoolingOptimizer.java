...
public class CarpoolingOptimizer {
    ...
    /*
     * TODO: Before adding the requests to the lists, try to find a way to randomly
     * shuffle the agents so each iteration, agents are not added in the same order.
    ...
    public CarpoolingOptimizer(BestRequestFinder bestRequestFinder, RequestsFilter requestsFilter,
            ArrayList<CarpoolingRequest> driversRequests, ArrayList<CarpoolingRequest> passengersRequests) {
    ...
    public HashMap<CarpoolingRequest, CarpoolingRequest> match() {
        ...
        for (int i = 0; i < driversRequests.size(); i++) {
            ...
            ArrayList<CarpoolingRequest> filteredPassengersRequests = requestsFilter.filterRequests(passengersRequests,
                    driverOrigin, driverDestination, driverDepartureTime);
